...
## 상태바 알림 아이콘 (흰 네모로 보일 때)

상태바의 작은 아이콘은 **웹 쪽 `public/sw.js` 의 `badge`** 가 정합니다. 크롬은 badge 가 있으면
앱의 SMALL_ICON 보다 그것을 우선 쓰고, 불투명한 부분을 전부 흰색으로 칠합니다. 그래서 색 있는
앱 아이콘을 badge 로 주면 흰 네모가 됩니다. 지금은 `public/badge-96.png`(투명 배경 + 흰 집 실루엣)
를 씁니다. 앱 쪽 `res/drawable-*/ic_notification.png` 는 badge 가 없을 때의 예비용으로 같은 그림입니다.
둘 다 바꾸려면 흰 실루엣 PNG 로만 교체하세요.
