...
## 상태바 알림 아이콘은 PNG 여야 합니다

`res/drawable-*/ic_notification.png` (흰 실루엣 + 투명 배경, 24dp 기준 밀도별 5장). 크롬이 앱에서
이 아이콘을 비트맵으로 읽어 가므로 **벡터(XML)를 넣으면 흰 네모로 보입니다.** 지금 그림은
코드로 그린 단순한 집 모양입니다. 바꾸려면 같은 이름의 PNG 5장으로 교체하면 됩니다.
